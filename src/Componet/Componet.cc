...
static CComponet* pComponet = CComponet::GetInstance();
...
CComponet::CComponet()
{
	...
	if (GyroNums <= 0)
	{
		std::cout << "GyroNums ֵ= " << GyroNums << "error" << std::endl;
		exit(0);
	}
	if (FlywheelNums<= 0)
	{
		std::cout << "FlywheelNums ֵ= " << FlywheelNums << "error" << std::endl;
		exit(0);
	}
	if (MagSensorNums <= 0)
	{
		std::cout << "MagSensorNums ֵ= " << MagSensorNums << "error" << std::endl;
		exit(0);
	}

	if (StarSensorNums <= 0)
	{
		std::cout << "StarSensorNums ֵ= " << StarSensorNums << "erroe" << std::endl;
		exit(0);
	}
	if (SunSensorNums <= 0)
	{
		std::cout << "SunSensorNums ֵ= " << SunSensorNums << "error" << std::endl;
		exit(0);
	}
	if (GnssNums <= 0)
	{
		std::cout << "GnssNums ֵ= " << GnssNums << "error" << std::endl;
		exit(0);
	}
	...
	for (size_t i{ 0 }; i < GyroNums; i++)
	{
		std::string GytoStr = "/Gyro/InstallMatrix" + std::to_string(i + 1);
		Gyros[i].InstallMatrix = pCfg->GetMat(GytoStr);
		Gyros[i].SamplePeriod = pCfg->Get<double>("/Gyro/SamplePeriod");
	}

	Wheels.resize(FlywheelNums);
	for (size_t i{0}; i < FlywheelNums; i++)
	{
		std::string FlywheelStr = "/Flywheel/InstallVector" + std::to_string(i + 1);
		Wheels[i].InstallVet = pCfg->GetVec(FlywheelStr);
	}

	SunSensors.resize(SunSensorNums);
	for (size_t i{0}; i < SunSensorNums; i++)
	{
		std::string SunSensorStr = "/SunSensor/InstallMatrix" + std::to_string(i + 1);
		SunSensors[i].InstallMatrix = pCfg->GetMat(SunSensorStr);
		SunSensors[i].SamplePeriod = pCfg->Get<double>("/SunSensor/SamplePeriod");
	}

	StarSensors.resize(StarSensorNums);
	for (size_t i{0}; i < StarSensorNums; i++)
	{
		std::string StarSensorStr = "/StarSensor/InstallMatrix" + std::to_string(i + 1);
		StarSensors[i].InstallMatrix.DcmData = pCfg->GetMat(StarSensorStr);
		StarSensors[i].SamplePeriod = pCfg->Get<double>("/StarSensor/SamplePeriod");
	}

	MagSensors.resize(MagSensorNums);
	for (size_t i{0}; i < MagSensorNums; i++)
	{
		std::string MagSensorStr = "/MagSensor/InstallMatrix" + std::to_string(i + 1);
		MagSensors[i].InstallMatrix = pCfg->GetMat(MagSensorStr);
		MagSensors[i].SamplePeriod = pCfg->Get<double>("/MagSensor/SamplePeriod");
	}

	GNSSs.resize(GnssNums);
	for (size_t i{0}; i < GnssNums; i++)
	{
		GNSSs[i].SamplePeriod = pCfg->Get<double>("/Gnss/SamplePeriod");
	}
}
